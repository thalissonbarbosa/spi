...
$imovel = $LocatarioImovel->getList("WHERE cl_locatarios_id = '{$id}'");
$fiador = $FiadorAssoc->getAssoc("WHERE cl_locatarios_id = '{$id}'");
$fiador_display = (!$fiador) ? 'display: none;' : '';
?>
    ...
    <div class="box-body">
        ...
        <ul class="list-group">
            ...
            <div id="pessoal">
                ...
                <li class="list-group-item clearfix no-padding">
                    <div class="list-title bg-light-blue-active"><strong>Declarante:</strong></div>
                    <span class="list-text"><?= $result['declarante'] ?></span>
                </li>
            ...
            <div id="fiador" style="<?= $fiador_display ?>">
                <li class="list-group-item bg-light-gray">Fiador</li>
                <li class="list-group-item clearfix">
                    <ul class="list-group" style="float:left; margin-left: 10px;">
                        <?php
                        if ($fiador):
                            foreach ($fiador as $data):
                                // dados do fiador
                                $detalhes = $Fiador->get($data['id']);
                                ?>
                                <li class="list-group-item">
                                    <a href="<?= URL_ROOT; ?>clientes/fiadores/detalhes?f=<?= $detalhes['id'] ?>" target="_blank"><?= $detalhes['nome'] ?></a>
                                </li>
                                <?php
                            endforeach;
                        endif;
            ...
            <div id="imovel">
                <li class="list-group-item bg-light-gray">Imóvel</li>
                <li class="list-group-item clearfix">
                    <ul class="list-group" style="float:left; margin-left: 10px;">
                        <?php
                        if ($imovel):
                            $i = 1;
                            foreach ($imovel as $data):
                                // dados do imovel
                                $detalhes = $Imovel->get($data['codigo_imovel']);
                                ...
                                ?>
                                <li class="list-group-item">
                                    <a href="<?= URL_ROOT; ?>imovel/detalhes/<?= $detalhes['codigo'] ?>" target="_blank"><?= $detalhes['codigo'] ?></a>
                                    <?= (!empty($detalhes['condominio'])) ? '- ' . $detalhes['condominio'] : '' ?>
                                    <span class="text-olive"><?= (!empty($detalhes['napt'])) ? '- Nº ' . $detalhes['napt'] : '' ?></span>
                                </li>
                                <?php
                                $i += 1;
                            endforeach;
                        else:
                            echo '<span class="list-text">Sem imóvel</span>';
                        endif;
                        ?>
                    </ul>
                </li>
